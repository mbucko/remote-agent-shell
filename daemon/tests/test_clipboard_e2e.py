...
@pytest.fixture
def device_id():
    """Test device ID for handle_message calls."""
...
class TestHappyPathImages:
    ...
    @pytest.mark.asyncio
    async def test_01_small_image_single_chunk_jpeg(self, manager, device_id, send_message, mock_clipboard, send_keys):
        """1. Small image < 64KB (single chunk) - JPEG"""
        image_data = b"\xff\xd8\xff\xe0" + b"\x00" * 1000  # JPEG header + data

        # Start
        await manager.handle_message(device_id, ClipboardMessage(
        ...
        send_keys.assert_called_with(device_id, "M-v")

        # Check complete message
        complete_msg = [m[0][1] for m in send_message.call_args_list if get_message_type(m[0][1]) == "complete"]
        assert len(complete_msg) == 1
        assert complete_msg[0].complete.content_type == ContentType.IMAGE

    @pytest.mark.asyncio
    async def test_02_small_image_single_chunk_png(self, manager, device_id, send_message, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_03_medium_image_multiple_chunks(self, manager, device_id, send_message, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_04_image_at_5mb_limit(self, manager, device_id, send_message, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_05_gif_format(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_06_webp_format(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_07_progress_updates_per_chunk(self, manager, device_id, send_message):
        ...
        progress_msgs = [m[0][1] for m in send_message.call_args_list if get_message_type(m[0][1]) == "progress"]
        assert len(progress_msgs) == 1
        assert progress_msgs[0].progress.received_chunks == 1
        assert progress_msgs[0].progress.total_chunks == 2

    @pytest.mark.asyncio
    async def test_08_confirmation_after_paste(self, manager, device_id, send_message, mock_clipboard):
        ...
        complete_msgs = [m[0][1] for m in send_message.call_args_list if get_message_type(m[0][1]) == "complete"]
        assert len(complete_msgs) == 1
        assert complete_msgs[0].complete.transfer_id == "img-008"

    @pytest.mark.asyncio
    async def test_09_state_cleared_after_success(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_10_can_start_new_after_complete(self, manager, device_id, mock_clipboard):
...
class TestHappyPathText:
    ...
    @pytest.mark.asyncio
    async def test_11_small_text_immediate(self, manager, device_id, mock_clipboard, send_keys):
    ...
    @pytest.mark.asyncio
    async def test_12_text_exactly_100kb(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_13_large_text_requires_approval(self, manager, device_id, send_message):
        ...
        approval_msgs = [m[0][1] for m in send_message.call_args_list if get_message_type(m[0][1]) == "approval_required"]
        assert len(approval_msgs) == 1
        assert approval_msgs[0].approval_required.size == len(text.encode())

    @pytest.mark.asyncio
    async def test_14_approved_text_pasted(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_15_text_with_unicode(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_16_text_with_newlines(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_17_text_only_whitespace(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_18_approval_preview_truncated(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_19_text_confirmation_sent(self, manager, device_id, send_message, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_20_text_keystroke_sent(self, manager, device_id, send_keys):
...
class TestChunkingEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_21_exactly_64kb_single_chunk(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_22_64kb_plus_one_byte(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_23_chunks_out_of_order(self, manager, device_id, mock_clipboard):
        ...
        await manager.handle_message(device_id, ClipboardMessage(image_chunk=ImageChunk(transfer_id="t", index=2, data=b"CCC")))
        await manager.handle_message(device_id, ClipboardMessage(image_chunk=ImageChunk(transfer_id="t", index=0, data=b"AAA")))
        await manager.handle_message(device_id, ClipboardMessage(image_chunk=ImageChunk(transfer_id="t", index=1, data=b"BBB")))

        assert mock_clipboard.last_image_data == b"AAABBBCCC"

    @pytest.mark.asyncio
    async def test_24_last_chunk_smaller(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_25_duplicate_chunk_idempotent(self, manager, device_id, mock_clipboard):
        ...
        await manager.handle_message(device_id, ClipboardMessage(image_chunk=ImageChunk(transfer_id="t", index=0, data=b"AAA")))
        await manager.handle_message(device_id, ClipboardMessage(image_chunk=ImageChunk(transfer_id="t", index=0, data=b"BBB")))  # Dup
        await manager.handle_message(device_id, ClipboardMessage(image_chunk=ImageChunk(transfer_id="t", index=1, data=b"CCC")))

        assert mock_clipboard.last_image_data == b"BBBCCC"

    @pytest.mark.asyncio
    async def test_26_single_byte_chunks(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_27_many_chunks(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_28_progress_every_chunk(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_29_progress_bytes_accurate(self, manager, device_id, send_message):
        """29. Progress bytes count accurate"""
        await manager.handle_message(device_id, ClipboardMessage(
            image_start=ImageStart(transfer_id="t", total_size=15, format=ImageFormat.PNG, total_chunks=3)
        ))
        await manager.handle_message(device_id, ClipboardMessage(image_chunk=ImageChunk(transfer_id="t", index=0, data=b"12345")))
    ...
    @pytest.mark.asyncio
    async def test_30_progress_shows_percentage(self, manager, device_id, send_message):
        ...
        await manager.handle_message(device_id, ClipboardMessage(image_chunk=ImageChunk(transfer_id="t", index=0, data=b"12345")))
        await manager.handle_message(device_id, ClipboardMessage(image_chunk=ImageChunk(transfer_id="t", index=1, data=b"12345")))
...
class TestSizeLimitErrors:
    ...
    @pytest.mark.asyncio
    async def test_31_image_over_5mb(self, manager, device_id, send_message):
        ...
        error_msgs = [m[0][1] for m in send_message.call_args_list if get_message_type(m[0][1]) == "error"]
        assert len(error_msgs) == 1
        assert error_msgs[0].error.code == ErrorCode.SIZE_EXCEEDED

    @pytest.mark.asyncio
    async def test_32_image_zero_size(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_33_image_negative_size(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_34_text_empty(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_35_chunk_zero_chunks(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_36_chunk_negative_chunks(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_37_chunk_over_64kb(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_38_text_100kb_plus_1(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_39_size_mismatch(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_40_state_cleared_on_size_error(self, manager, device_id, send_message):
...
class TestFormatValidation:
    ...
    @pytest.mark.asyncio
    async def test_41_jpeg_accepted(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_42_png_accepted(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_43_gif_accepted(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_44_webp_accepted(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_45_unspecified_format_rejected(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_46_format_preserved_in_backend(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_47_format_in_complete_message(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_48_state_cleared_on_format_error(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_49_can_retry_after_format_error(self, manager, device_id, mock_clipboard, send_message):
    ...
    @pytest.mark.asyncio
    async def test_50_format_error_message_clear(self, manager, device_id, send_message):
...
class TestChunkIndexErrors:
    ...
    @pytest.mark.asyncio
    async def test_51_negative_index(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_52_index_too_large(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_53_index_way_too_large(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_54_chunk_without_start(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_55_chunk_wrong_transfer_id(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_56_valid_index_after_error(self, manager, device_id, mock_clipboard, send_message):
    ...
    @pytest.mark.asyncio
    async def test_57_all_valid_indices_work(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_58_error_message_includes_index(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_59_transfer_cleared_on_index_error(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_60_boundary_index(self, manager, device_id, mock_clipboard):
        """60. Boundary index (n-1) works"""
        await manager.handle_message(device_id, ClipboardMessage(
            image_start=ImageStart(transfer_id="t", total_size=6, format=ImageFormat.PNG, total_chunks=3)
        ))
        await manager.handle_message(device_id, ClipboardMessage(image_chunk=ImageChunk(transfer_id="t", index=0, data=b"AA")))
        await manager.handle_message(device_id, ClipboardMessage(image_chunk=ImageChunk(transfer_id="t", index=1, data=b"BB")))
        await manager.handle_message(device_id, ClipboardMessage(image_chunk=ImageChunk(transfer_id="t", index=2, data=b"CC")))
...
class TestCancelScenarios:
    ...
    @pytest.mark.asyncio
    async def test_61_cancel_mid_transfer(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_62_cancel_sends_ack(self, manager, device_id, send_message):
        ...
        cancelled_msgs = [m[0][1] for m in send_message.call_args_list if get_message_type(m[0][1]) == "cancelled"]
        assert len(cancelled_msgs) == 1
        assert cancelled_msgs[0].cancelled.transfer_id == "t"

    @pytest.mark.asyncio
    async def test_63_cancel_when_idle(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_64_double_cancel(self, manager, device_id, send_message):
        """64. Double cancel handled gracefully"""
        await manager.handle_message(device_id, ClipboardMessage(
            image_start=ImageStart(transfer_id="t", total_size=100, format=ImageFormat.PNG, total_chunks=10)
        ))
        await manager.handle_message(device_id, ClipboardMessage(image_cancel=ImageCancel(transfer_id="t")))
        await manager.handle_message(device_id, ClipboardMessage(image_cancel=ImageCancel(transfer_id="t")))
    ...
    @pytest.mark.asyncio
    async def test_65_chunk_after_cancel_rejected(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_66_cancel_no_clipboard_set(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_67_cancel_no_paste_sent(self, manager, device_id, send_keys):
    ...
    @pytest.mark.asyncio
    async def test_68_can_start_after_cancel(self, manager, device_id, mock_clipboard):
        ...
        await manager.handle_message(device_id, ClipboardMessage(image_cancel=ImageCancel(transfer_id="t1")))
    ...
    @pytest.mark.asyncio
    async def test_69_cancel_wrong_id_ignored(self, manager, device_id, mock_clipboard):
        ...
        await manager.handle_message(device_id, ClipboardMessage(image_cancel=ImageCancel(transfer_id="wrong")))
...
class TestClipboardErrors:
    ...
    @pytest.mark.asyncio
    async def test_71_clipboard_set_failure(self, manager, device_id, send_message, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_72_clipboard_error_clears_state(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_73_paste_key_failure(self, manager, device_id, send_message, send_keys):
    ...
    @pytest.mark.asyncio
    async def test_75_error_includes_transfer_id(self, manager, device_id, send_message, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_76_can_retry_after_clipboard_error(self, manager, device_id, mock_clipboard, send_message):
    ...
    @pytest.mark.asyncio
    async def test_77_error_message_descriptive(self, manager, device_id, send_message, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_78_paste_error_after_clipboard_set(self, manager, device_id, send_message, mock_clipboard, send_keys):
    ...
    @pytest.mark.asyncio
    async def test_79_image_clipboard_error(self, manager, device_id, send_message, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_80_text_clipboard_error(self, manager, device_id, send_message, mock_clipboard):
...
class TestTimeoutScenarios:
    ...
    @pytest.mark.asyncio
    async def test_87_timeout_cancelled_on_complete(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_90_no_timeout_for_text(self, manager, device_id, mock_clipboard):
...
class TestConcurrentState:
    ...
    @pytest.mark.asyncio
    async def test_91_only_one_transfer_at_time(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_92_text_during_image_transfer(self, manager, device_id, send_message, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_93_rapid_text_pastes(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_94_rapid_image_transfers(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_95_state_isolated_between_transfers(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_100_state_after_error(self, manager, device_id, mock_clipboard):
...
class TestPlatformKeystroke:
    ...
    async def test_101_macos_uses_cmd_v(self, default_config, send_message, send_keys, platform_info_macos, mock_clipboard):
        ...
        await manager.handle_message(device_id, ClipboardMessage(text_paste=TextPaste(text="test")))
        send_keys.assert_called_with(device_id, "M-v")
    ...
    async def test_102_linux_uses_ctrl_v(self, default_config, send_message, send_keys, platform_info_linux_x11, mock_clipboard):
        ...
        await manager.handle_message(device_id, ClipboardMessage(text_paste=TextPaste(text="test")))
        send_keys.assert_called_with(device_id, "C-v")
    ...
    async def test_103_config_override_keystroke(self, send_message, send_keys, platform_info_macos, mock_clipboard):
        ...
        await manager.handle_message(device_id, ClipboardMessage(text_paste=TextPaste(text="test")))
        send_keys.assert_called_with(device_id, "C-S-v")

        manager.cleanup()

    @pytest.mark.asyncio
    async def test_104_keystroke_for_image(self, manager, device_id, send_keys):
    ...
    @pytest.mark.asyncio
    async def test_105_keystroke_for_approved_text(self, manager, device_id, send_keys):
    ...
    @pytest.mark.asyncio
    async def test_106_keystroke_not_sent_on_error(self, manager, device_id, send_keys, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_107_keystroke_after_clipboard_set(self, manager, device_id, mock_clipboard, send_keys):
        ...
        async def tracked_send_keys(dev_id, keys):
    ...
    @pytest.mark.asyncio
    async def test_108_keystroke_once_per_paste(self, manager, device_id, send_keys):
    ...
    @pytest.mark.asyncio
    async def test_109_keystroke_once_per_image(self, manager, device_id, send_keys):
    ...
    @pytest.mark.asyncio
    async def test_110_multiple_pastes_multiple_keystrokes(self, manager, device_id, send_keys):
        """110. Each paste sends its own keystroke"""
        for i in range(3):
            await manager.handle_message(device_id, ClipboardMessage(text_paste=TextPaste(text=f"text{i}")))
...
class TestMessageCallbacks:
    ...
    @pytest.mark.asyncio
    async def test_111_progress_callback_per_chunk(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_112_complete_callback_on_success(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_113_error_callback_on_failure(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_114_cancelled_callback_on_cancel(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_115_approval_callback_for_large_text(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_117_message_order_correct(self, manager, device_id, send_message):
        ...
        msg_types = [get_message_type(m[0][1]) for m in send_message.call_args_list]
        assert msg_types == ["progress", "progress", "complete"]

    @pytest.mark.asyncio
    async def test_118_all_fields_populated(self, manager, device_id, send_message):
        ...
        progress_msg = [m[0][1] for m in send_message.call_args_list if get_message_type(m[0][1]) == "progress"][0]
    ...
    @pytest.mark.asyncio
    async def test_119_complete_has_type(self, manager, device_id, send_message):
        ...
        complete_msg = [m[0][1] for m in send_message.call_args_list if get_message_type(m[0][1]) == "complete"][0]
        assert complete_msg.complete.content_type == ContentType.IMAGE

    @pytest.mark.asyncio
    async def test_120_text_complete_type(self, manager, device_id, send_message):
...
class TestEncodingEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_121_binary_image_data(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_122_null_bytes_in_image(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_123_unicode_text(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_124_emoji_text(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_125_control_chars_in_text(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_126_very_long_single_line(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_127_many_short_lines(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_128_mixed_line_endings(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_129_special_json_chars(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_130_size_utf8_calculated(self, manager, device_id, send_message):
...
class TestAdditionalEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_131_empty_transfer_id(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_132_long_transfer_id(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_133_unicode_transfer_id(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_134_whitespace_only_transfer_id(self, manager, device_id, mock_clipboard):
    ...
    @pytest.mark.asyncio
    async def test_136_max_int_total_chunks(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_137_progress_0_to_100_percent(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_138_first_chunk_is_index_zero(self, manager, device_id, send_message):
    ...
    @pytest.mark.asyncio
    async def test_139_logging_doesnt_crash(self, manager, device_id, mock_clipboard, caplog):
    ...
    @pytest.mark.asyncio
    async def test_140_all_error_codes_distinct(self, manager, device_id, send_message):
