...
# sources: auth.proto, daemon.proto, lan_direct.proto, ntfy.proto, ntfy_signaling.proto, pairing.proto, qr_payload.proto, sessions.proto, signaling.proto, terminal.proto
...
@dataclass(eq=False, repr=False)
class LanDirectAuthRequest(betterproto.Message):
    """
    ...
    timestamp: int = betterproto.int64_field(2)
    """Unix timestamp in seconds (must be within ±30s of server time)"""
...
@dataclass(eq=False, repr=False)
class LanDirectAuthResponse(betterproto.Message):
...
@dataclass(eq=False, repr=False)
class PairRequest(betterproto.Message):
    """Phone -> Daemon: Request to pair"""

    device_id: str = betterproto.string_field(1)
    device_name: str = betterproto.string_field(2)
    auth_proof: bytes = betterproto.bytes_field(3)
    """
    ...
    nonce: bytes = betterproto.bytes_field(4)
    """
    32-byte random nonce — prevents replay, daemon uses to prove knowledge back
    """
...
@dataclass(eq=False, repr=False)
class PairResponse(betterproto.Message):
    """Daemon -> Phone: Pairing response"""

    daemon_device_id: str = betterproto.string_field(1)
    hostname: str = betterproto.string_field(2)
    device_type: "DeviceType" = betterproto.enum_field(3)
    auth_proof: bytes = betterproto.bytes_field(4)
    """
...
class NtfySignalMessage(betterproto.Message):
    ...
    pair_request: "PairRequest" = betterproto.message_field(10)
    """
    Pairing credential exchange (used with PAIR_REQUEST/PAIR_RESPONSE types)
    """

    pair_response: "PairResponse" = betterproto.message_field(11)
