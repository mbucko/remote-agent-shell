...
### Phase 1: Initial Pairing (QR Code + Credential Exchange)

First-time setup requires physical access to the laptop. Pairing is **decoupled from connecting** — it only exchanges credentials, it does NOT establish a WebRTC connection.
...
│                                                                 │
│  1. Scans QR code                                               │
│  2. Derives session_id, ntfy_topic, auth_key from secret        │
│  3. Sends PAIR_REQUEST via ntfy (encrypted)                     │
│  4. Receives PAIR_RESPONSE with daemon info                     │
│  5. Stores master secret in Android Keystore                    │
│  6. Pairing complete! (no WebRTC connection yet)                │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
**Pairing Credential Exchange:**

After scanning QR, the phone and daemon exchange credentials via ntfy (or direct HTTP if on same LAN). This is a single-round HMAC-authenticated exchange — no WebRTC involved.
...
**HMAC Computation:**
- Request: `HMAC(auth_key, "pair-request" || session_id || device_id || nonce)`
- Response: `HMAC(auth_key, "pair-response" || nonce)`
- Nonce: 32 random bytes generated by phone, prevents replay

### Phase 2: P2P Connection (WebRTC)

After pairing is complete, the phone connects to the daemon via WebRTC. This happens separately from pairing — either immediately after, or later when the user opens the app.
...
└────┬─────┘                                          └────┬─────┘
     │                                                     │
     │  1. Discover daemon via mDNS or ntfy DISCOVER       │
     │                                                     │
     │  2. Create WebRTC offer                             │
     │                                                     │
     │  3. Send offer via ntfy or POST /signal/{device_id} │
     │─────────────────────────────────────────────────────▶
     │                                                     │
     │  4. Laptop creates answer                           │
     │                                                     │
     │  5. Receive answer via ntfy or HTTP response        │
     │◀────────────────────────────────────────────────────│
     │                                                     │
...
| `/api/pair/{session_id}/complete` | POST | Pairing credential exchange |
| `/signal/{session_id}` | POST | Exchange SDP offer/answer (pairing) |
| `/reconnect/{device_id}` | POST | Exchange SDP offer/answer (reconnection) |

### Phase 3: Authentication Handshake

After P2P connection is established, both sides authenticate using the shared secret. This 4-step mutual auth happens over the WebRTC DataChannel during connection (not during pairing).
...
ntfy.sh is used for pairing credential exchange, WebRTC signaling relay, and IP change notifications.

| Setting | Value |
|---------|-------|
| Server | `ntfy.sh` (public) or self-hosted |
| Topic | Derived from master secret: `"ras-" + sha256(secret)[:12]` |
| Payload | AES-GCM encrypted protobuf |
...
| Type | Purpose |
|------|---------|
| `PAIR_REQUEST` | Phone → Daemon: credential exchange during pairing |
| `PAIR_RESPONSE` | Daemon → Phone: credential exchange response |
| `OFFER` | Phone → Daemon: WebRTC SDP offer for connection |
| `ANSWER` | Daemon → Phone: WebRTC SDP answer |
| `DISCOVER` | Phone → Daemon: discover daemon address |
...
│   ├── pairing.proto         # PairRequest/PairResponse for credential exchange
│   └── ntfy_signaling.proto  # ntfy signaling messages (OFFER, ANSWER, PAIR_REQUEST, etc.)
...
Pairing and connecting are separate state machines:
