...
1. **Pairing authentication** — single-round HMAC exchange during initial pairing (via ntfy)
2. **Connection authentication** — 4-step mutual handshake during WebRTC connection

### Pairing Authentication (HMAC Exchange)

During initial pairing, the phone and daemon authenticate via a single-round HMAC exchange. No WebRTC connection is established — this happens over ntfy (encrypted).
...
**Security properties:**
- **Mutual authentication:** Both sides prove knowledge of `auth_key` (derived from `master_secret`)
- **Replay protection:** 32-byte random nonce generated by phone; same nonce bound into both HMACs
- **Domain separation:** Request uses `"pair-request"` prefix, response uses `"pair-response"` prefix
- **Encrypted transport:** All messages encrypted with AES-GCM via ntfy signaling layer

### Connection Authentication (4-Step Handshake)

After P2P connection is established via WebRTC, both sides prove they know the shared secret. This happens over the WebRTC DataChannel.
